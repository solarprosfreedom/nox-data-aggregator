  ...
  return (
    <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600">
      ...
      <div className="flex items-center gap-1.5">
        <button className={btn} onClick={() => goTo(1)} disabled={current <= 1} aria-label="First page">«</button>
        <button className={btn} onClick={() => goTo(current - 1)} disabled={current <= 1} aria-label="Previous page">‹</button>
        {pages.map((p) => (
          <button
            key={p}
            onClick={() => goTo(p)}
        ...
        <button className={btn} onClick={() => goTo(current + 1)} disabled={current >= totalPages} aria-label="Next page">›</button>
        <button className={btn} onClick={() => goTo(totalPages)} disabled={current >= totalPages} aria-label="Last page">»</button>
      </div>
